...
const { primitives, primitivesDark, semantic, paletteSemantics, typography, spacing } = tokens;
```
...
Palette semantics (`palette-semantics.json`) use the same shade references in both modes. Mode-specific cases (e.g. `brand-yellow` hex, foreground/background flips) belong in consumer repos.
